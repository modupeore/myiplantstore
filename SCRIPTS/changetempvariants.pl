...
my $Mfolder; my %VCFhash; my %VEPhash; my %AMISG; my %AMIST;
...
# 		
			...
			else { $annotation = undef; $annotationfile = "NULL"; $annfileversion = "NULL"; }
...
# 			$sth ->execute($lib_id, $refgenome, $annotationfile, $annfileversion, $stranded,$sequences,$_[2] );
			...
			#variant analysis
			VARIANTS($first, $accepted, $refgenome, $annotation);
...
sub VEPVARIANT {
	...
	my %GENEname = ''; my %GeneEntrezinfo = ''; my $position; my %location = '';
	...
	foreach my $abc (sort keys %VCFhash) {
		foreach my $def (sort {$a <=> $b} keys %{ $VCFhash{$abc} }) {
			...
			if ($vcf[3] =~ /,/){
				if (length($vcf[2]) == length($first)){
				...
				elsif (length($vcf[2]) < length($first)) {
					...
					exit "False variant, cross-check asap";
				}
			}
			elsif (length $vcf[2] == length $vcf[3]){
				$itvariants++; $itsnp++; $variantclass = "SNV";
			...
			my $entrez = "-";
			print OUTDBVAR "$abc\t$def\t$vcf[2]\t$vcf[3]\t$vcf[4]\t$variantclass\t";
			print OUTDBVAR "$vep[1]\t$entrez\t$vep[9]\t$vep[2]\t";
			print OUTDBVAR "$vep[0]\t$vep[8]\t$vep[3]\t$vep[4]\t";
			print OUTDBVAR "$vep[5]\t$vep[6]\t$vcf[5]\t$vep[7]\n";

# 			$sth = $dbh->prepare("insert into variants_result ( library_id, chrom, position, ref_allele, alt_allele, quality, variant_class,
# 			ensembl_gene, entrez_gene, gene_name, transcript, feature, gene_type,
# 			consequence, protein_position, protein, codon_change, zygosity, existing_variant ) values
# 			(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
# 			$sth ->execute($libnumber, $abc, $def, $vcf[2], $vcf[3], $vcf[4], $variantclass, $vep[1], $entrez, $vep[9], $vep[2], $vep[0], $vep[8], 
# 			$vep[3], $vep[4], $vep[5], $vep[6], $vcf[5], $vep[7]);
		}
	}
	close (OUTDBVAR);
	#VARIANT_SUMMARY
...
sub SNPdatVARIANT {
	...
	my %Featureinfo = ''; my %Proinfo = ''; my %Prochangeinfo = ''; my %Codoninfo = '';
	my %dbSNPinfo = ''; my %locinfo = ''; my %Entrezinfo = ''; my %GENEtype = '';
	my %GENEname = ''; my $position; my %location = ''; my ($consqofgene,$idofgene);
	...
	foreach my $chr (@variantfile){
		unless ($chr =~ /^#/){
			...
			elsif (((split(':', $chrdetail[9]))[0]) eq '1/2'){$verd = "heterozygous alternate";}
			#VCFhash information
			$VCFhash{$chrdetail[0]}{$chrdetail[1]} = "$chrdetail[0]|$chrdetail[1]|$chrdetail[3]|$chrdetail[4]|$chrdetail[5]|$verd";
		}
	}
	...
	foreach my $chr2 (@file){
		...
		if ($chrdetails[10] =~ /ID=AMISG/){
			...
			foreach (@geneidlist){
				if($_ =~ /^ID=AMISG/){$idofgene = substr($_,3);}last;
			}	
			...
			my $codonchange = "$aq$dq$bq/$aq$eq$bq";
			my $aminoacid = substr(substr($chrdetails[20],1),0,-1);
			...
			if($chrdetails[21] == "N") {
			...
			elsif ($chrdetails[21] == "Y"){$consqofgene = "SYNONYMOUS_CODING";}
			elsif($chrdetails[21] == "NA") {
				...
				exit "There's a problem with the consequence of the gene\n\n";
			}
			
			foreach (0..$#chrdetails){if ($chrdetails[$_] eq "NA"){ $chrdetails[$_] = "NULL";}} #converting NA to NULL
	
		#GENE - 1
			if (exists $Geneinfo{$chrdetails[0]}{$chrdetails[1]}){
				if ($Geneinfo{$chrdetails[0]}{$chrdetails[1]} eq $idofgene){
					$Geneinfo{$chrdetails[0]}{$chrdetails[1]} = $idofgene;
				}
				else {
					my $tempdetails = "$Geneinfo{$chrdetails[0]}{$chrdetails[1]},$idofgene";
					$Geneinfo{$chrdetails[0]}{$chrdetails[1]} = $tempdetails;
				}
			}
			else {	
		...
		#TRANSCRIPT - 2
			if (exists $Transcriptinfo{$chrdetails[0]}{$chrdetails[1]}){
				if ($Transcriptinfo{$chrdetails[0]}{$chrdetails[1]} eq $AMIST{$idofgene}){
					$Transcriptinfo{$chrdetails[0]}{$chrdetails[1]} = $AMIST{$idofgene};
				}
				else {
					my $temp2details = "$Transcriptinfo{$chrdetails[0]}{$chrdetails[1]},$AMIST{$idofgene}";
					$Transcriptinfo{$chrdetails[0]}{$chrdetails[1]} = $temp2details;
				}
			}
			else {	
		...
		#CONSEQUENCE - 3
			if (exists $Conqinfo{$chrdetails[0]}{$chrdetails[1]} ){
				if ($Conqinfo{$chrdetails[0]}{$chrdetails[1]} eq $consqofgene){
					$Conqinfo{$chrdetails[0]}{$chrdetails[1]} = $consqofgene;
				}
				else {
					my $temp3details = "$Conqinfo{$chrdetails[0]}{$chrdetails[1]},$consqofgene";
					$Conqinfo{$chrdetails[0]}{$chrdetails[1]} = $temp3details;
				}			
			}
			else {	
		...
		#FEATURE TYPE - 5 #SNPdat
			if (exists $Featureinfo{$chrdetails[0]}{$chrdetails[1]}){
				if ($Featureinfo{$chrdetails[0]}{$chrdetails[1]} eq $chrdetails[5]){
					$Featureinfo{$chrdetails[0]}{$chrdetails[1]} = $chrdetails[5];
				}
				else {
					my $temp5details = "$Featureinfo{$chrdetails[0]}{$chrdetails[1]},$chrdetails[5]";
					$Featureinfo{$chrdetails[0]}{$chrdetails[1]} = $temp5details;
				}			
			}
			else {	
		...
		#PROTEIN POSITION - 6 #SNPdat this isn't provided
			if (exists $Proinfo{$chrdetails[0]}{$chrdetails[1]}){
				$Proinfo{$chrdetails[0]}{$chrdetails[1]} = "NULL";			
			}
			else {	
				$Proinfo{$chrdetails[0]}{$chrdetails[1]} = "NULL";
			}
		...
		#PROTEIN CHANGE - 7 (or Amino Acid) #SNPdat
			if (exists $Prochangeinfo{$chrdetails[0]}{$chrdetails[1]}){
				if ($Prochangeinfo{$chrdetails[0]}{$chrdetails[1]} eq $aminoacid){
					$Prochangeinfo{$chrdetails[0]}{$chrdetails[1]}= $aminoacid;
				}
				else {
					my $temp7details = "$Prochangeinfo{$chrdetails[0]}{$chrdetails[1]},$aminoacid";
					$Prochangeinfo{$chrdetails[0]}{$chrdetails[1]} = $temp7details;
				}			
			}
			else {	
		...
		#CODON CHANGE - 8 #SNPdat
			if (exists $Codoninfo{$chrdetails[0]}{$chrdetails[1]}){
				if ($Codoninfo{$chrdetails[0]}{$chrdetails[1]} eq $codonchange){
					$Codoninfo{$chrdetails[0]}{$chrdetails[1]} = $codonchange;
				}
				else {
					my $temp8details = "$Codoninfo{$chrdetails[0]}{$chrdetails[1]},$codonchange";
					$Codoninfo{$chrdetails[0]}{$chrdetails[1]} = $temp8details;
				}			
			}
			else {	
		...
		#dbSNP - 9 #SNPdat # no dbSNP info for alligator
			if (exists $dbSNPinfo{$chrdetails[0]}{$chrdetails[1]}){
				$dbSNPinfo{$chrdetails[0]}{$chrdetails[1]} = "NULL";			
			}
			else {	
				$dbSNPinfo{$chrdetails[0]}{$chrdetails[1]} = "NULL";
			}			
		...
		#GENE name - 10 #name of gene from the alligator gene list. #SNPdat
			if (exists $GENEname{$chrdetails[0]}{$chrdetails[1]}){
				if ($GENEname{$chrdetails[0]}{$chrdetails[1]} eq $AMISG{$idofgene}){
					$GENEname{$chrdetails[0]}{$chrdetails[1]} = $AMISG{$idofgene};
				}
				else {
					my $temp10details = "$GENEname{$chrdetails[0]}{$chrdetails[1]},$AMISG{$idofgene}";
					$GENEname{$chrdetails[0]}{$chrdetails[1]} = $temp10details;
				}
			}
			else {	
		...
		#GENE type - 11 #SNPdat
			if (exists $GENEtype{$chrdetails[0]}{$chrdetails[1]}){
				if ($GENEtype{$chrdetails[0]}{$chrdetails[1]} eq $chrdetails[3]){
					$GENEtype{$chrdetails[0]}{$chrdetails[1]} = $chrdetails[3];
				}
				else {
					my $temp11details = "$GENEtype{$chrdetails[0]}{$chrdetails[1]},$chrdetails[3]";
					$GENEtype{$chrdetails[0]}{$chrdetails[1]} = $temp11details;
				}
			}
			else {	
	...
	foreach my $alldetails (keys %location){
		...
		#FINAL OUTPUT
		$VEPhash{$chrdetails1}{$chrdetails2} = "$clean2|$clean3|$clean4|$clean5|$clean6|$clean7|$clean8|$clean9|$clean10|$clean11";
	}
}
sub CLEANUP {
