...
#CREATING LOG FILES
my $std_out = '/home/modupeore17/.LOG/DB-'.`date +%m-%d-%y_%T`; chomp $std_out; $std_out = $std_out.'.log';
my $std_err = '/home/modupeore17/.LOG/DB-'.`date +%m-%d-%y_%T`; chomp $std_err; $std_err = $std_err.'.err';
...
foreach my $NewFolder (@NewDirectory) { 
	...
	foreach my $SubNewFolder (@Subdirectory) {
		if ($SubNewFolder =~ /^\w.*_(\d.*)$/){
			CHECKING();
			if($1 == 68){#unless (exists $Hashresults{$1}){
