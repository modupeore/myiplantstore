...
# #CREATING LOG FILES
my $std_out = '/home/modupeore17/.LOG/TransDB-'.`date +%m-%d-%y_%T`; chomp $std_out; $std_out = $std_out.'.log';
my $std_err = '/home/modupeore17/.LOG/TransDB-'.`date +%m-%d-%y_%T`; chomp $std_err; $std_err = $std_err.'.err';
...
sub PARSING {
	print "\n\tINSERTING TRANSCRIPTS INTO THE DATABASE : \t library_$_[0]\t".`date`."\n\n";
	...
	if ($numberz[0] == $first){
 	...
 	# PARSER FOR transcripts_summary TABLE
		...
		else { print "it isnt chicken rather it is $allgeninfo[$ref]\n";}
	}
	else {print "library_id dont match $numberz[0] == $first\n";}
}
sub HTSEQ {
	print "\n\tSTORING HTSEQ IN THE DATABASE\t".`date`."\n\n";
...
sub VARIANTS{
	print "\n\tWORKING ON VARIANT ANALYSIS\t".`date`."\n\n";
...
sub DBVARIANTS{	
	print "\n\tINSERTING VARIANTS INTO THE DATABASE\t".`date`."\n\n";
