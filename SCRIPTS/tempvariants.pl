...
# #CREATING LOG FILES
my $std_out = '/home/modupeore17/.LOG/bamvar-'.`date +%m-%d-%y_%T`; chomp $std_out; $std_out = $std_out.'.log';
my $std_err = '/home/modupeore17/.LOG/bamvar-'.`date +%m-%d-%y_%T`; chomp $std_err; $std_err = $std_err.'.err';
my $jobid = "GECO-".`date +%m-%d-%y_%T`;
...
$in1 = "/home/modupeore17/bam_fileS";
...
# RESULTS_HASH
my %Hashresults; my %Birdresults; my %Variantresults; my %HTSEQ;
...
foreach my $filer (@Directory){
CHECKING();
	if ($filer =~ /^library_(\d*)_.*bam$/){ print $1;
		unless (exists $Variantresults{$1}){
			if (exists $Hashresults{$1}){
				VARIANTS($1, "$in1/$filer", "chicken");
				#progress report
...
sub CHECKING {
	...
	while ($row = $sth->fetchrow_array() ) {
        ...
        while ($row = $sth->fetchrow_array() ) {
        my $newrow = substr($row,0,-2);
                $Variantresults{$newrow} = $number; $number++;
        }
}
sub PARSING {
	...
	if ($numberz[0] == $first){
		...
		else {die "File format doesn't match what was encoded for frnak_metadata\nSyntax error:\n\t@allgeninfo\n$mystderror\n";}
		if ($allgeninfo[$ref] =~ /chicken/){
 	...
 	# PARSER FOR transcripts_summary TABLE
		...
		#PARSING FOR SNPanalysis
			...
			my $replace; my $temptest = `head -n 1 $allgeninfo[$ann]`;
			...
			my $sequences; #initializing fastq file used
					my $stranded = $allgeninfo[$str]; # (stranded or not)
			my $annotationfile = uc ( (split('\.',((split("\/", $allgeninfo[$ann]))[-1])))[-1] ); #(annotation file)
			my $refgenome = (split('\/', $allgeninfo[$ref]))[-1]; #reference genome name
			my $annfileversion = substr(`head -n 1 $allgeninfo[$ann]`,2,-1); #annotation file version
...
sub VARIANTS{
	...
	my $testpath = `ls $Mfolder`;
		$Mfolder = "/home/modupeore17/CHICKENvariants/$libraryNO";
...
next;}
	...
	#SPLIT&TRIM
	my $splittrim = "java -jar $GATKDIR -T SplitNCigarReads -R $REF -I $Mfolder/".$libraryNO."_mdup.bam -o $Mfolder/".$libraryNO."_split.bam -rf ReassignOneMappingQuality -RMQF 255 -RMQT 60 --filter_reads_with_N_cigar --allow_potentially_misencoded_quality_scores";
	`$splittrim`;
	
	#GATK
	my $gatk = "java -jar $GATKDIR -T HaplotypeCaller -R $REF -I $Mfolder/".$libraryNO."_split.bam -o $Mfolder/$libraryNO.vcf --allow_potentially_misencoded_quality_scores";
	`$gatk`;
